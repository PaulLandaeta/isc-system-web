...
const validationSchema = Yup.object({
  ...
  total_hours: Yup.number().when("isIntern", (isIntern, schema) => isIntern ? schema.required("Las horas becarias son obligatorias") : schema.nullable()),
});
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (isIntern) {
          const internUsername = rest.email.split('@')[0];
        ...
        const errorMessage = error.response?.data?.message || error.message || 'Error creando estudiante';
